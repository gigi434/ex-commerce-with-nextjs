 ...
 * 商品投稿フォーム
 */
// コールバック関数に対して型を指定する場合、割り当てられる関数の型を定義する必要がある。このことを呼び出しシグネチャと呼ぶ
// (onProductSave)にすると呼び出しシグネチャが必要になるが、onProductFormPropsでは呼び出しシグネチャを定義しているわけではなく
// アロー関数として定義しているため、({onProductSave})としてプロパティを参照し変数として引数を設定しなければならない

// 呼び出しシグネチャとはどのような関数かを表現する型定義であり、似たような型注釈を記載する必要がなくなる。
// onProductSaveの場合はプロパティとして宣言しているため、
