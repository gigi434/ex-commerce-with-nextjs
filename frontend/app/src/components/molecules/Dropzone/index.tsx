...
/**
 * ドラッグをしているか判定する
 * @params value ドラッグしているオブジェクト
 * @returns ドラッグアンドドロップしているオブジェクトがあればTrue
 */
...
const getFilesFromEvent = (e: React.DragEvent | React.ChangeEvent): File[] => {
  // もし何らかのオブジェクトをドラッグしている場合はそのファイルを新しい配列オブジェクトに格納する
  if (isDragEvt(e)) {
    return Array.from(e.dataTransfer.files)
    // イベントから入力されたファイルがnullでなく、ファイルとして追加されたら追加されたファイルを新しい配列オブジェクトに格納する
